 ...
 * kx修改20160604
 * 新增attr属性，自定义属性值  ,格式固定为attr:val,attr:val,
 * 返回方法，可以返回text和attr;
							...
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li" data-hover="true">' + htmlCreateMenu(obj.data) +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'" data-attr="'+attr+'"><i class="'+ clKey +'triangle"></i>'+ text +'</a>' + 
									'</li>';
								} else {
									htmlMenuLi = '<li class="'+ clKey +'li">' +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'" data-attr="'+attr+'">'+ text +'</a>' + 
									'</li>';
								};
								htmlMenu += htmlMenuLi;
			...
			if (!jqueryMenu.size()) {
				...
				$(idKey + params.name +" a").bind("click", function() {
					...
					if ($.isFunction(callback)) {
						callback.call(D.data("trigger"),attr);	
					}
					$.smartMenu.hide();
					return false;
				});
