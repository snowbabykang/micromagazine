...
;(function ($) {
    ...
    searchBar.prototype = {
        ...
        init: function () {
            //console.log('init');
			this.pre = this.options.sid + '_' + this.options.uid;
        ...
        initTags: function (type) {
            ...
            if (type === 'his') {
                temp = window.localStorage.getItem(_this.pre + '_hisTags');
                if (temp) this.setTags('his', temp.split(','));
            }

            //自定义获取标签数据
            this.options.getTagsData[type] && this.options.getTagsData[type](_this, function(arry) {
        ...
        addHisTagData: function(text) { //添加历史搜索数据
            ...
            window.localStorage.setItem(this.pre + '_hisTags', this.tags['his'].toString());

            return this;
        },
        
        clearHisTagData: function() { //清空历史搜索数据
			this.options.onhisClear();
            this.$tag['his'].html('').hide();
            this.tags['his'] = [];
            window.localStorage.setItem(this.pre + '_hisTags', '');
            return this;
        },
